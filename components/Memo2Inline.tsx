...
export default function Memo2Inline() {
  ...
  const loadContents = async () => {
    try {
      const response = await fetch('/api/memo2');
      if (response.ok) {
        const data = await response.json();
        setContents(data);
      }
    } catch (error) {
      console.error('메모2 로드 실패:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      let imageUrl = "";
      if (selectedImage) {
        imageUrl = await uploadFile(selectedImage, "memo2");
      }
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl">메모2를 불러오는 중...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="container mx-auto py-8 px-4 relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">특별한 메모와 프로젝트 아이디어를 기록하세요</p>
        </div>
          ...
          <div className="text-center mb-8">
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="max-w-2xl bg-gray-900 text-white">
                <DialogHeader>
                  <DialogTitle>메모2 작성</DialogTitle>
                  <DialogDescription>
                    새로운 메모2를 작성하거나 기존 메모2를 편집하세요.
                  </DialogDescription>
                </DialogHeader>
        ...
        {contents.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-400 text-lg">아직 작성된 메모2가 없습니다.</p>
            {isAuthenticated && (
              <p className="text-gray-500 text-sm mt-2">새 메모2를 작성해보세요!</p>
            )}
          </div>
        )}
      </div>
